...
class ManagerAgent:
    ...
    '''
    def create_micro_levels(self, level, goal_positions):
        print("initial level")
        ...
        for agent in agents_positions:
            ...
            micro_level = self.create_level(level, agent, nearest_object, nearest_goal)
            micro_levels.append(micro_level)
            # print micro level
            self.print_level(micro_level)
    ...
    def create_level(self, level, agent_pos, object_pos, goal_pos):
        new_level = []
        for i in range(len(level)):
            row = []
            for j in range(len(level[i])):
...
manager_agent.create_micro_levels(env.level, env.goal_positions_list)
